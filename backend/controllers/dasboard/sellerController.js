...
class SellerController {
  request_seller_get = async (req, res) => {
    ...
    try {
      if (searchValue) {
        // טיפול במקרה של חיפוש (ניתן להוסיף קוד)
      } else {
  ...
  get_active_sellers = async (req, res) => {
    ...
    try {
      if (searchValue) {
        const sellers = await sellerModel.find({ $text: { $search: searchValue }, status: "active" })
          .skip(skipPage)
          .limit(parPage)
          .sort({ createdAt: -1 });
        const totalSeller = await sellerModel.find({ $text: { $search: searchValue }, status: "active" }).countDocuments();
  ...
  get_deactive_sellers = async (req, res) => {
    ...
    try {
      if (searchValue) {
        const sellers = await sellerModel.find({ $text: { $search: searchValue }, status: "deactive" })
          .skip(skipPage)
          .limit(parPage)
          .sort({ createdAt: -1 });
        const totalSeller = await sellerModel.find({ $text: { $search: searchValue }, status: "deactive" }).countDocuments();
        responseReturn(res, 200, { totalSeller, sellers });
      } else {
