  ...
  add_category = async (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
      if (err) {
        ...
        try {
          const result = await cloudinary.v2.uploader.upload(image.filepath, { folder: "categorys" });
          if (result) {
    ...
    form.parse(req, async (err, fields, files) => {
      if (err) {
        ...
        try {
          let result = null;
          if (image) {
            result = await cloudinary.v2.uploader.upload(image.filepath, { folder: "categorys" });
          }
